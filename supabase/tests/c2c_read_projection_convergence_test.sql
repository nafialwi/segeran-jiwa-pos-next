...
begin
    ...
    if jsonb_array_length(v_history) < 1 then
        raise exception 'C2C_V2_HISTORY_FAILED';
    end if;

    select row -> 'items' -> 0
    into v_first_item
    from jsonb_array_elements(v_history) row
    where row ->> 'sale_id' in (v_sale1::text,v_sale2::text)
    limit 1;

    if v_first_item is null
       or v_first_item ->> 'display_name' <> 'C2B Make To Order'
       or v_first_item ->> 'variant_name' <> 'Regular'
       or v_first_item ->> 'variant_id' <> '00000000-0000-0000-0000-000000009602'
       or nullif(v_first_item ->> 'stock_item_id','') is not null then
        raise exception 'C2C_V2_HISTORY_FAILED';
    end if;

    v_history := public.transaction_history_search(
        p_product => 'C2B Direct Stock',
        p_limit => 100
    );
    ...
    if not exists (
        select 1
        from jsonb_array_elements(v_history) row
        where row -> 'items' -> 0 ->> 'display_name' = 'C2B Direct Stock'
          and row -> 'items' -> 0 ->> 'stock_item_id' = '00000000-0000-0000-0000-000000009503'
          and nullif(row -> 'items' -> 0 ->> 'variant_id','') is null
    ) then
        raise exception 'C2C_LEGACY_HISTORY_FAILED';
    end if;

    v_report := public.report_run('PRODUCT',current_date,current_date);

    if position('C2B Make To Order' in v_report::text)=0
       or position('Regular' in v_report::text)=0
       or position('C2B Direct Stock' in v_report::text)=0 then
        raise exception 'C2C_PRODUCT_REPORT_FAILED';
    end if;

    v_preview := public.sale_correction_preview(v_sale2);

    if coalesce((v_preview #>> '{stock,tracked_lines}')::integer,0) <> 2 then
        raise exception 'C2C_CORRECTION_PREVIEW_FAILED';
    end if;
end $$;
