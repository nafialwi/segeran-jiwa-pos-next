...
-- C3-B facts: discount, cash tender/change, line note, zero-total inventory-only sales.

do $$
declare
    v_location uuid;
    v_result jsonb;
    v_sale_discount uuid;
    v_sale_free_refund uuid;
    v_sale_free_correction uuid;
begin
    set local role authenticated;
    perform set_config(
        'request.jwt.claims',
    ...
    from public.locations
    where code='GERAI'
      and business_id=(select id from public.businesses where code='SJ');
    ...
    v_result := public.checkout_sale_v2(
        ...
        'C3B discounted cash sale'
    );

    v_sale_discount := (v_result ->> 'sale_id')::uuid;

    if (v_result ->> 'subtotal')::numeric <> 10000
       or (v_result ->> 'discount_amount')::numeric <> 2000
       or (v_result ->> 'total_amount')::numeric <> 8000
       or (v_result ->> 'tendered_amount')::numeric <> 10000
       or (v_result ->> 'change_amount')::numeric <> 2000 then
        raise exception 'C3B_TENDER_DISCOUNT_RESULT_FAILED';
    end if;
    ...
    v_result := public.checkout_sale_v2(
        ...
        jsonb_build_object(
            'type','PERCENT',
            'value',100,
            'reason','Sampling 100 persen'
        ),
        'C3B free sale refund path'
    );
    v_sale_free_refund := (v_result ->> 'sale_id')::uuid;
    ...
    v_result := public.checkout_sale_v2(
        ...
        jsonb_build_object(
            'type','PERCENT',
            'value',100,
            'reason','Sampling koreksi'
        ),
        'C3B free sale correction path'
    );
    v_sale_free_correction := (v_result ->> 'sale_id')::uuid;

    perform set_config('c3b.sale_discount',v_sale_discount::text,true);
    perform set_config('c3b.sale_free_refund',v_sale_free_refund::text,true);
    perform set_config('c3b.sale_free_correction',v_sale_free_correction::text,true);

    perform public.refund_sale(
        v_sale_free_refund,
        'RETURN_TO_STOCK',
        'NONE',
        'C3B zero total refund',
        'C3B:REFUND:FREE'
    );

    perform public.correct_sale(
        v_sale_free_correction,
        'C3B zero total correction',
        'C3B:CORRECT:FREE'
    );
end $$;
...
do $$
declare
    v_sale_discount uuid := current_setting('c3b.sale_discount')::uuid;
    v_sale_free_refund uuid := current_setting('c3b.sale_free_refund')::uuid;
    v_sale_free_correction uuid := current_setting('c3b.sale_free_correction')::uuid;
    v_invoice text;
    v_count integer;
    v_qty numeric;
    v_history jsonb;
begin
    if not exists (
        select 1
        from public.sales s
        where s.id=v_sale_discount
          and s.subtotal=10000
          and s.discount_type='PERCENT'
          and s.discount_value=20
          and s.discount_amount=2000
          and s.total_amount=8000
          and s.discount_reason='Promo C3B'
          and s.discount_approved_by='00000000-0000-0000-0000-000000009301'
    ) then
        raise exception 'C3B_DISCOUNT_FACT_FAILED';
    end if;

    if not exists (
        select 1
        from public.payments p
        where p.sale_id=v_sale_discount
          and p.method='CASH'
          and p.amount=8000
          and p.tendered_amount=10000
          and p.change_amount=2000
    ) then
        raise exception 'C3B_TENDER_CHANGE_FACT_FAILED';
    end if;

    if not exists (
        select 1
        from public.sale_items li
        where li.sale_id=v_sale_discount
          and li.line_note='Tanpa sedotan'
    ) then
        raise exception 'C3B_LINE_NOTE_FAILED';
    end if;

    select invoice_number into v_invoice
    from public.sales where id=v_sale_free_refund;
    ...
    if v_count <> 1 then
        raise exception 'C3B_FULL_DISCOUNT_INVENTORY_FAILED';
    end if;

    select count(*) into v_count
    from public.money_movements mm
    where mm.source_type='SALE'
      and mm.source_ref=v_invoice
      and mm.reason_code='SALE_PAYMENT';

    if v_count <> 0 then
        raise exception 'C3B_FULL_DISCOUNT_MONEY_FAILED';
    end if;

    if not exists (
        select 1
        from public.sale_refunds r
        where r.sale_id=v_sale_free_refund
          and r.sale_total=0
          and r.payout_amount=0
          and r.refund_method='NONE'
          and r.inventory_movement_id is not null
          and r.payout_money_movement_id is null
    ) then
        raise exception 'C3B_REFUND_ZERO_TOTAL_FAILED';
    end if;

    if not exists (
        select 1
        from public.sale_corrections c
        where c.sale_id=v_sale_free_correction
          and c.sale_total=0
          and c.inventory_reversal_movement_id is not null
          and c.money_reversal_movement_id is null
          and c.cash_adjustment_transaction_id is null
    ) then
        raise exception 'C3B_CORRECTION_ZERO_TOTAL_FAILED';
    end if;

    select invoice_number
    into v_invoice
    from public.sales
    where id=v_sale_discount;
    ...
    v_history := public.transaction_history_search(
        p_invoice_number => v_invoice,
        p_limit => 10
    );

    if position('Tanpa sedotan' in v_history::text)=0
       or position('Promo C3B' in v_history::text)=0
       or position('"tendered_amount": 10000' in v_history::text)=0
       or position('"change_amount": 2000' in v_history::text)=0 then
        raise exception 'C3B_HISTORY_FACTS_FAILED';
    end if;
    ...
    where business_id=(select id from public.businesses where code='SJ')
      and location_id=(select id from public.locations where code='GERAI' and business_id=(select id from public.businesses where code='SJ'))
      and stock_item_id='00000000-0000-0000-0000-000000009501';

    if v_qty <> 0.950 then
        raise exception 'C3B_FINAL_INGREDIENT_BALANCE_FAILED';
    end if;
    ...
    if v_qty <> 9.000 then
        raise exception 'C3B_FINAL_PACKAGING_BALANCE_FAILED';
    end if;
end $$;
